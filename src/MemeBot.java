...
public class MemeBot extends ListenerAdapter {
    ...
    private final String[] airhorns = { "!airhorn default", "!airhorn reverb", "!airhorn spam", "!airhorn tripletap",
            "!airhorn fourtap", "!airhorn distant", "!airhorn echo", "!airhorn clownfull", "!airhorn clownshort",
            "!airhorn clownspam", "!airhorn highfartlong", "!airhorn highfartshort", "!airhorn midshort",
            "!airhorn truck", "!anotha one", "!anotha one_classic", "!anotha one_echo", "!cena airhorn", "!cena full",
            "!cena jc", "!cena nameis", "!cena spam", "!eb areyou_classic", "!eb areyou_condensed", "!eb areyou_crazy",
            "!eb areyou_ethan", "!eb classic", "!eb echo", "!eb high", "!eb slowandlow", "!eb cuts", "!eb beat",
            "!eb sodiepop", "!stan herd", "!stan moo", "!stan x3", "!bday horn", "!bday horn3", "!bday sadhorn",
            "!bday weakhorn", "!wtc" };
    // map of commands
    ...
    public static void main(String[] args) {
        ...
        try {
            JDA jda = new JDABuilder(AccountType.BOT).setToken(token).addListener(new MemeBot()).buildBlocking();
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
    ...
    /**
     * Do stuff when a user connects to a voice channel in a guild (discord
     * server). In this instance, the bot will join the channel, send a random
     * airhorn solutions bot command, wait to connect, then disconnect.
     * 
     * @param event
     *            the GuildVoiceJoinEvent that is associated with this event.
     *            from the event the following can be extracted: the user/member
     *            who joined, the voice channel they joined, and the guild the
     *            voice channel was in, among other things.
     */
    @Override
    public void onGuildVoiceJoin(GuildVoiceJoinEvent event) {
        dc = false;
        if (airhornOn) {
            // ensure that we do not disconnect immediately after beginning a
            // connection
    ...
    /**
     * Execute code when the bot detects a message sent to a guild message
     * channel.
     * 
     * @param event
     *            the event associated with the message. from the event the
     *            following can be extracted: the message, the user/member who
     *            sent it, the content of the message, the message channel it
     *            was sent in, and the guild it was sent in, among other things.
     */
    @Override
     ...
     *
     * @param channel
     *            the voicechannel to connect to.
     */
    ...
    /**
     * Wait to disconnect from the audio connection. If the bot disconnects
     * before the connection is fully connected, the bot will be stuck in the
     * channel. As such, we wait until the bot is connected before
     * disconnecting. NOTE: must be run in a separate thread.
     */
    ...
    /**
     * Parse bot commands. Takes in the bot commands and performs the expected
     * actions.
     * 
     * @param com
     *            the command to be processed.
     * @param user
     *            the user issuing the command. Used to determine permissions
     *            for restricted commands.
     * @param chan
     *            the MessageChannel that the command is given in. Used for the
     *            bot to respond to the commands.
     */
    private void parseCommands(BotCommand com, Member user, MessageChannel chan) {
        /*
         * Most of the restricted commands (like shutdown) are restricted to
         * users with the "Botnet Managers" role. As such, when parsing
         * commands, we check if the user making the request has that role
         * before allowing them to use those restricted commands.
         */
        ...
        case MEISENNERD: // !MemeBot meisennerd
            if (user.getUser().getId().equals("107272630842728448")) {
                try {
                    ...
                    chan.sendFile(stream,"meis.png", null).queue();
                } catch (Exception e) {
                    e.printStackTrace();
                }
            }
        ...
        case CHRISTMAS: // !MemeBot christmas
            try {
                InputStream stream = MemeBot.class.getResourceAsStream("reindeer.png");
                chan.sendFile(stream,"reindeer.png", null).queue();
            } catch (Exception e) {
                e.printStackTrace();
            }
            break;
     ...
     * There are two HashMaps that store command data: commands and
     * commandDescriptions. commands maps the string representation of the
     * command to its associated BotCommand enum type, for determining which
     * command to parse. commandDescroptions maps the BotCommand enum types to a
     * String containing a description of the command, for usage in the com+desc
     * command.
     */
    private static void addCommands() {
        if (commands == null && commandDescriptions == null) {
            ...
            commandDescriptions.put(BotCommand.AIRHORN_COMMANDS_DESCRIPTIONS,
                    "Prints a list of MemeBot commands and their descriptions");
    ...
    /**
     * Print the list of commands to a given MessageChannel. Note: ordering is
     * based on the internal ordering of HashMap.
     * 
     * @param mc
     *            the MessageChannel to output to.
     */
    private void printCommands(MessageChannel mc) {
        ...
        for (String c : commands.keySet()) {
            /*
             * add the commands to a temp string. If the command is restricted,
             * the command will be underlined (surrounded by __ in discord
             * markdown). In addition, the command itself will be placed inside
             * of a code block (surrounded by ` in discord).
             */
            temp = temp + "\n" + (commands.get(c).isRestricted() ? "__" : "") + "`" + c + "`"
                    + (commands.get(c).isRestricted() ? "__" : "");
        }
        // send the message
    ...
    /**
     * Print the list of commands and their descriptions to a given
     * MessageChannel. Note: ordering is based on the internal ordering of
     * HashMap. Descriptions will only be printed if they exist.
     * 
     * @param mc
     *            the MessageChannel to output to.
     */
    ...
    /**
     * Print a list of Airhorn Solutions commands. Note: prints the list of
     * commands used internally. Does not automatically update if the Airhorn
     * Solutions bot updates
     * 
     * @param mc
     *            the MessageChannel to output to.
     */
...
/**
 * An implementation of ConnectionListener designed to send the instance of
 * MemeBot a signal to disconnect. To do this, we use OnStatusChange.
 * 
...
class MemeListener implements ConnectionListener {
    ...
    /**
     * Execute code on change in ping (presumably). Note: currently unused
     * 
     * @param arg0
     *            the ping to discord
     */
    ...
    /**
     * Execute code when the ConnectionStatus changes. This is used to determine
     * when the bot should try to disconnect from the voice channel. When the
     * bot tries to disconnect from the voice channel before being connected,
     * the bot becomes stuck in the voice channel.
     * 
     * @param arg0
     */
    @Override
     ...
     *            the user speaking
     * @param arg1
     *            if the user is speaking or not
     */
    @Override
...
/**
 * Enum specifying the type of command being sent by the user. Contains a
 * boolean value restricted, denoting if the command is restricted in use,
 * either to specific people or specific roles. To access this data, call
 * isRestricted() on the enum.
 * 
 * @author Daniel Cormier
 * @author Cosmo Viola
 */
enum BotCommand {
    ...
    AIRHORN_ON(true), AIRHORN_OFF(true), AIRHORN_STATUS(false), MEISENNERD(true), COMMAND_LIST(false), SHUTDOWN(
            true), AIRHORN_COMMANDS(false), AIRHORN_COMMANDS_DESCRIPTIONS(false), CHRISTMAS(false);
